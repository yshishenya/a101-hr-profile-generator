...
def validate_department_name(department: str) -> str:
    ...
    """
    department = validate_required_string(department, "department", min_length=3, max_length=200)
...
def validate_position_name(position: str) -> str:
    ...
    """
    position = validate_required_string(position, "position", min_length=3, max_length=200)
...
def validate_profile_id(profile_id: str) -> str:
    ...
    try:
        ...
        raise ValidationError(
            ...
            field="profile_id",
            details={"format": "UUID4", "example": "123e4567-e89b-12d3-a456-426614174000"}
        )
...
def validate_json_data(data: Dict[str, Any], schema_name: str) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValidationError(
            f"{schema_name} must be a JSON object",
            field=schema_name
        )
    
    if len(data) == 0:
        raise ValidationError(
            f"{schema_name} cannot be empty",
            field=schema_name
        )
