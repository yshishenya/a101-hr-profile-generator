...
class LLMClient:
    ...
    def __init__(self,
        ...
        if self.langfuse_public_key and self.langfuse_secret_key:
            ...
            logger.warning("⚠️ Langfuse credentials not found in config - tracing disabled")
            self.langfuse = None
    ...
    def _create_generation_with_prompt(self, prompt, messages, model, temperature, max_tokens, response_format, trace_metadata=None):
        """
        ...
        if hasattr(response, 'usage') and response.usage:
            ...
            post_metadata = {
                **enriched_metadata,
                ...
                "provider": response.model_extra.get('provider', 'openrouter') if hasattr(response, 'model_extra') and response.model_extra else 'openrouter',
                ...
                "response_length": len(response.choices[0].message.content) if response.choices and response.choices[0].message else 0,
                ...
                "openai_sdk_version": response.__class__.__module__ if hasattr(response.__class__, '__module__') else 'unknown'
            }
            ...
            try:
                if hasattr(usage, 'model_extra') and usage.model_extra:
                    if 'prompt_tokens_details' in usage.model_extra:
                        post_metadata["prompt_tokens_cached"] = usage.model_extra['prompt_tokens_details'].get('cached_tokens', 0)
                    if 'completion_tokens_details' in usage.model_extra:
                        completion_details = usage.model_extra['completion_tokens_details']
                        post_metadata["completion_reasoning_tokens"] = completion_details.get('reasoning_tokens', 0)
                        post_metadata["completion_image_tokens"] = completion_details.get('image_tokens', 0)
    ...
    def generate_profile_from_langfuse(self,
        ...
        try:
            ...
            if not self.langfuse:
                raise ValueError("Langfuse не инициализирован - невозможно получить промпт")
            ...
            generation_time = time.time() - start_time
            logger.error(f"Langfuse generation failed after {generation_time:.2f}s: {e}")
    ...
    def validate_profile_structure(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        required_fields = [
            "responsibility_areas", "professional_skills", "corporate_competencies"
        ]
