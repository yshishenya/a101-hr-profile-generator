...
class PromptManager:
    ...
    def __init__(self, 
        ...
        self.prompt_registry = {
            "profile_generation": {
                ...
                "variables": [
                    "company_map", "org_structure", "department", "position",
                    "department_path", "profile_examples", "kpi_data", 
                    "it_systems", "json_schema", "employee_name",
                    "generation_timestamp", "data_version"
                ],
                "config": {
                        ...
                        "json_schema": {
                            ...
                            "schema": {
                                ...
                                "properties": {
                                    ...
                                    "category": {"type": "string", "enum": ["Специалист", "Линейный руководитель (группа, направление)", "Руководитель среднего уровня (отдел, управление)", "Руководитель высшего уровня (департамент)"]},
                                    ...
                                    "professional_skills": {
                                        ...
                                        "items": {
                                            ...
                                            "properties": {
                                                ...
                                                "target_level": {"type": "string", "enum": ["Базовый", "Продвинутый", "Экспертный"]}
                                            },
                                    ...
                                    "corporate_competencies": {
                                        "type": "array",
                                        "items": {"type": "string", "enum": ["Инновационность и развитие", "Ориентация на результат", "Стратегическое видение и принятие решений", "Клиентоориентированность", "Эффективная коммуникация", "Работа в команде", "Лидерство"]}
                                    },
                                    "personal_qualities": {
                                        "type": "array",
                                        "items": {"type": "string", "enum": ["Внимательность", "Ответственность", "Коммуникабельность", "Стрессоустойчивость", "Настойчивость", "Исполнительность", "Системность мышления", "Инициативность", "Проактивность", "Критическое мышление", "Лидерство", "Аналитический склад ума", "Многозадачность", "Решительность"]}
                                    }
                                },
                                "required": ["position_title", "department_broad", "department_specific", "category", "primary_activity", "responsibility_areas", "professional_skills", "corporate_competencies", "personal_qualities"],
    ...
    def _get_cache_status(self, prompt_name: str) -> Dict[str, Any]:
        ...
        matching_keys = [k for k in self._prompt_cache.keys() if k.startswith(prompt_name)]
    ...
    def _clear_cache_for_prompt(self, prompt_name: str):
        ...
        keys_to_remove = [k for k in self._prompt_cache.keys() if k.startswith(prompt_name)]
        
        for key in keys_to_remove:
            self._prompt_cache.pop(key, None)
            self._cache_timestamps.pop(key, None)
        
        logger.info(f"Cleared cache for prompt '{prompt_name}' ({len(keys_to_remove)} entries)")
    
    def validate_prompt_template(self, template: str, prompt_name: str) -> Dict[str, Any]:
        """
