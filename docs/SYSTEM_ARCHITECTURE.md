...
```
[CatalogService.get_departments()] 
                ↓
    [Проверка кеша (TTL: 1 час)]
                ↓
         [Кеш валиден?] ─── ДА → [Возврат из кеша: 3ms]
                ↓ НЕТ
    [DataLoader.load_full_organization_structure()]
                ↓
    [Загрузка 510 департаментов + 4376 должностей: 24ms]
                ↓
    [Генерация позиций по типам департаментов]
                ↓
    [Сохранение в память кеш + БД персистентность]
                ↓
    [Возврат результата: 40ms общее время]
```
...
- **Масштабируемость:** Ready для роста количества департаментов
