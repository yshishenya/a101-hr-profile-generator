...
class EnhancedExcelToMDConverter:
    ...
    def detect_kpi_sections(self, rows_data):
        ...
        for i, row in enumerate(rows_data):
            ...
            if any(keyword in row_text.lower() for keyword in 
                   ['корпоративные кпэ', 'личные кпэ', 'бюджетные кпэ', 'кпэ', 'показател']):
                if current_section:
    ...
    def parse_kpi_table(self, data_rows):
        ...
        for i, (row_idx, row) in enumerate(data_rows):
            non_empty_cells = [self.clean_text(cell) for cell in row if self.clean_text(cell)]
            
            if not non_empty_cells:
                continue
            
            # First meaningful row might be headers
            if i == 0 or (len(non_empty_cells) >= 3 and 
                         any(keyword in ' '.join(non_empty_cells).lower() 
            ...
            if len(non_empty_cells) >= 2:
                kpi_item = {
                    ...
                    'target_value': non_empty_cells[1] if len(non_empty_cells) > 1 else '',
                ...
                if len(headers) > 3:
                    for j, header in enumerate(headers[3:], start=3):
                        if j < len(non_empty_cells):
                            if any(keyword in header.lower() for keyword in ['методика', 'описание', 'источник']):
                                kpi_item['description'] += f"{header}: {non_empty_cells[j]}; "
                
                kpi_items.append(kpi_item)
        
        return kpi_items, headers
    
    def convert_sheet_to_structured_md(self, worksheet, sheet_name, dept_name, employee_name):
        ...
        if sections:
            for section in sections:
                ...
                if kpi_items:
                    for kpi in kpi_items:
                        ...
                        if kpi['target_value']:
                            md_content.append(f"- **Целевое значение:** {kpi['target_value']}")
                        if kpi['unit']:
                            md_content.append(f"- **Единица измерения:** {kpi['unit']}")
                        if kpi['description']:
                            md_content.append(f"- **Дополнительная информация:** {kpi['description'].strip('; ')}")
                        
                        if kpi['weights']:
                            md_content.append("- **Веса/Коэффициенты:**")
                            for i, weight in enumerate(kpi['weights']):
                                if weight and str(weight).strip():
                                    header_name = headers[i + 3] if i + 3 < len(headers) else f"Позиция {i+1}"
                    ...
                    for _, row in section['data']:
                        non_empty = [self.clean_text(cell) for cell in row if self.clean_text(cell)]
                        if len(non_empty) >= 2:
            ...
            for i, row in enumerate(rows_data):
                ...
                if len(non_empty) >= 2:
                    if i == 0 or any(keyword in ' '.join(non_empty).lower() 
                                   for keyword in ['кпэ', 'показател', 'цел']):
                        # Header row
                        md_content.append("| " + " | ".join(non_empty) + " |")
                        md_content.append("| " + " | ".join(["---"] * len(non_empty)) + " |")
                    else:
                        # Data row
                        md_content.append("| " + " | ".join(non_empty) + " |")
    ...
    def convert_file(self, excel_file):
        ...
        try:
            ...
            for sheet_name in workbook.sheetnames:
                ...
                data_rows = sum(1 for row in ws.iter_rows() if any(cell.value for cell in row))
