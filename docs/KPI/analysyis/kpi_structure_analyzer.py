...
def analyze_excel_structure(file_path):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            analysis["complexity_indicators"]["merged_cells_count"] += len(
                merged_ranges
            )
            ...
            for row in range(1, min(11, ws.max_row + 1)):
                ...
                for col in range(1, min(11, ws.max_column + 1)):
                    ...
                    if (
                        ...
                        and cell.fill.start_color.index
                    ):
                    ...
                    if cell.value is not None:
                        sheet_analysis["data_types_found"].add(
            ...
            sheet_analysis["data_types_found"] = list(
                sheet_analysis["data_types_found"]
            )
        ...
        try:
            ...
            for sheet_name, df in excel_data.items():
                ...
                print(
                    f"  Сложность заголовков: {'Высокая' if header_complexity['is_complex'] else 'Низкая'}"
                )
...
def analyze_header_structure(df):
    """Анализ структуры заголовков"""
    result = {"is_complex": False, "issues": []}
        ...
        for i in range(min(3, len(first_rows))):
            ...
            if non_null_count > 0 and non_null_count < len(row) * 0.7:
                ...
                result["issues"].append(
                    f"Возможен многоуровневый заголовок в строке {i+1}"
                )
...
def calculate_complexity_score(indicators):
    ...
    format_penalty = min(
        (indicators["different_fonts_count"] + indicators["different_fills_count"]) * 2,
        20,
    )
...
def generate_recommendations(analysis):
    ...
    if indicators["merged_cells_count"] > 10:
        recommendations.append(
            "Много объединенных ячеек - нужна специальная обработка структуры"
        )

    if indicators["complex_headers"]:
        recommendations.append(
            "Сложная структура заголовков - потребуется интеллектуальный парсинг"
        )

    if indicators["formulas_count"] > 5:
        recommendations.append(
            "Присутствуют формулы - нужно решить, использовать значения или формулы"
        )

    if indicators["different_fonts_count"] > 3:
        recommendations.append(
            "Разнообразное форматирование может содержать семантическую информацию"
        )

    complexity_score = analysis["complexity_score"]
    if complexity_score <= 20:
        recommendations.append(
            "✅ РЕКОМЕНДАЦИЯ: Достаточно простого Excel парсера (pandas/openpyxl)"
        )
    elif complexity_score <= 50:
        recommendations.append(
            "⚠️ РЕКОМЕНДАЦИЯ: Нужен продвинутый парсер с обработкой структуры"
        )
    else:
        recommendations.append(
            "🤖 РЕКОМЕНДАЦИЯ: Использовать LLM для интерпретации сложной структуры"
        )
...
def main():
    ...
    if all_analyses:
        avg_complexity = sum(a["complexity_score"] for a in all_analyses) / len(
            all_analyses
        )
        ...
        total_merged_cells = sum(
            a["complexity_indicators"]["merged_cells_count"] for a in all_analyses
        )
        total_formulas = sum(
            a["complexity_indicators"]["formulas_count"] for a in all_analyses
        )
