...
def analyze_data_samples():
    ...
    for filename in target_files:
        ...
        try:
            ...
            print(f"Размер: {main_sheet.max_row} строк x {main_sheet.max_column} столбцов")
            ...
            if merged_ranges:
                ...
                for i, mr in enumerate(merged_ranges[:8]):  # Показываем первые 8
                    ...
                    value = str(top_left.value)[:30] + "..." if top_left.value and len(str(top_left.value)) > 30 else str(top_left.value)
                    print(f"  {mr}: '{value}'")
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                if any(keyword.lower() in row_str.lower() for keyword in kpi_keywords):
                    print(f"  Строка {idx+1} (возможный заголовок): {row.values[:6].tolist()}")
            ...
            for idx, row in df.iterrows():
                if idx > 30:
                    break
                numeric_count = sum(1 for val in row.values if isinstance(val, (int, float)) and not pd.isna(val))
                if numeric_count >= 2:
