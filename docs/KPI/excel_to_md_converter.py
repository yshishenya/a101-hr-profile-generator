...
class ExcelToMDConverter:
    ...
    def analyze_sheet_structure(self, worksheet):
        ...
        for i, row in enumerate(rows_data):
            ...
            if any(keyword in row_text.lower() for keyword in ['kpi', 'кпэ', 'показател', 'цел', 'критери']):
    ...
    def convert_sheet_to_md(self, worksheet, sheet_name, dept_name, employee_name):
        ...
        for i, row in enumerate(rows_data):
            ...
            if (len([cell for cell in row if cell and str(cell).strip()]) <= 2 and 
                any(keyword in row_text.lower() for keyword in ['kpi', 'кпэ', 'показател', 'корпоратив', 'личн', 'цел'])):
            ...
            if (len(non_empty_cells) >= 3 and 
                any(keyword in ' '.join(non_empty_cells).lower() 
                    for keyword in ['показател', 'значени', 'описан', 'вес', 'цел', 'критери', 'ед.изм'])):
                
                table_headers = non_empty_cells
                md_content.append("| " + " | ".join(table_headers) + " |")
                md_content.append("| " + " | ".join(["---"] * len(table_headers)) + " |")
                in_table = True
                continue
            ...
            if len(non_empty_cells) >= 2:
                if in_table and table_headers:
                    ...
                    if len(non_empty_cells) == 1:
                        ...
                        md_content.append(f"**{non_empty_cells[0]}:** {' | '.join(non_empty_cells[1:])}")
    ...
    def convert_file(self, excel_file):
        ...
        try:
            ...
            # Convert to markdown
            md_content = self.convert_sheet_to_md(main_sheet, main_sheet.title, dept_name, employee_name)
