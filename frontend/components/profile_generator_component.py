...
class ProfileGeneratorComponent:
        ...
        with ui.column().classes("w-full space-y-4") as container:
            ...
            with ui.card().classes("w-full"):
                with ui.card_section():
                    ui.label("🎯 ГЕНЕРАТОР ПРОФИЛЕЙ (Smart Search)").classes("text-h5 font-bold text-primary mb-4")
        ...
        with ui.card().classes("w-full bg-blue-50"):
            with ui.card_section():
                with ui.row().classes("w-full items-center justify-between"):
                    ...
                    self.stats_label = ui.label("Загрузка статистики...").classes("text-gray-600")
    ...
    async def _render_search_section(self):
        ...
        with ui.row().classes("w-full items-center space-x-2 mb-4"):
            ...
            ui.button("🔍", on_click=self._trigger_search).props("flat").classes("text-primary")
        
        # Популярные теги
        with ui.row().classes("w-full items-center space-x-2 mb-4"):
            ui.label("ИЛИ Выбрать из популярных:").classes("text-body2 text-gray-600 min-w-fit")
            
            popular_searches = ["Менеджер", "Специалист", "Руководитель", "Директор", "Разработчик", "Аналитик"]
            for tag in popular_searches:
                ui.button(
                    f"#{tag}",
                    on_click=lambda t=tag: self._quick_search(t)
                ).props("size=sm outlined color=primary").classes("text-xs")
        
        # Контейнер живых результатов поиска
        with ui.column().classes("w-full"):
            ui.label("⚡ Live результаты:").classes("font-semibold text-gray-700").bind_visibility_from(
                self, "has_search_results"
            )
            self.search_results_container = ui.column().classes("w-full space-y-1 max-h-48 overflow-y-auto")
    
    async def _render_selected_position(self):
        """Отрисовка выбранной должности"""
        with ui.column().classes("w-full mt-4"):
            self.selected_position_display = ui.column().classes("w-full").bind_visibility_from(
                self, "has_selected_position"
            )
    
    async def _render_additional_params(self):
        """Отрисовка дополнительных параметров"""
        with ui.column().classes("w-full mt-4").bind_visibility_from(self, "has_selected_position"):
    ...
    async def _render_generate_button(self):
        ...
        with ui.row().classes("w-full justify-center mt-6"):
            ...
            ).props("size=lg color=primary").classes("font-bold px-8 py-3")
    ...
    async def _perform_search(self, query: str):
        ...
        try:
            ...
            if search_response.get("success"):
                ...
                await self._display_search_results(positions[:8])  # Показываем только первые 8 для живого поиска
            else:
                self._clear_search_results()
    ...
    async def _render_position_result(self, position: Dict):
        ...
        with ui.row().classes("w-full items-center justify-between p-2 hover:bg-gray-50 rounded-lg border-l-2 border-l-primary cursor-pointer").on(
            ...
            with ui.column().classes("flex-1"):
                with ui.row().classes("items-center space-x-2"):
                    ...
                    ui.chip(f"Ур.{position['level']}", color=level_color).props("size=xs")
        ...
        with self.selected_position_display:
            with ui.card().classes("w-full border-l-4 border-l-green-500 bg-green-50"):
                with ui.card_section():
                    with ui.row().classes("w-full items-center justify-between"):
                        with ui.column().classes("flex-1"):
                            ui.label(f"✅ ВЫБРАНО: {self.selected_position['name']}").classes("font-bold text-green-800")
                            
                            # Получаем путь департамента
                            dept_path = self.selected_position.get('department', 'Не указан')
                            ui.label(f"📍 Департамент: {dept_path}").classes("text-body2 text-green-700")
                            
                            # Дополнительная информация
                            level = self.selected_position.get('level', 'Не указан')
                            category = self.selected_position.get('category', 'Не указана')
                            ui.label(f"⚙️ Уровень: {level} • Категория: {category}").classes("text-caption text-green-600")
    ...
    async def _track_generation_progress(self):
        ...
        with self.progress_container:
            with ui.card().classes("w-full border-l-4 border-l-blue-500"):
                with ui.card_section():
                    ui.label("🔄 Генерация профиля...").classes("font-bold text-blue-800 mb-2")
                    
                    progress_bar = ui.linear_progress(value=0).classes("w-full")
                    status_label = ui.label("Инициализация...").classes("text-body2 text-blue-600")
    ...
    async def _handle_generation_complete(self):
        ...
        with self.progress_container:
            with ui.card().classes("w-full border-l-4 border-l-green-500 bg-green-50"):
                with ui.card_section():
                    ui.label("✅ Профиль успешно создан!").classes("font-bold text-green-800 mb-2")
    ...
    async def _handle_generation_error(self, error_message: str):
        ...
        with self.progress_container:
            with ui.card().classes("w-full border-l-4 border-l-red-500 bg-red-50"):
                with ui.card_section():
                    ui.label("❌ Ошибка генерации").classes("font-bold text-red-800 mb-2")
