...
class DashboardComponent:
    ...
    def _create_page_header(self):
        ...
        with ui.row().classes("w-full items-center justify-between mb-6"):
            ui.label("🏢 Генерация профилей должностей").classes(
                "text-h4 text-weight-medium"
            )

            # Кнопка обновления
            ui.button(icon="refresh", on_click=self._refresh_dashboard).props(
                "flat round"
        ...
        with ui.card().classes("w-full mb-6"):
            ...
            with self.stats_card:
                ...
                with ui.card().classes("flex-1 text-center p-4"):
                    ui.label(str(total_positions)).classes(
                        "text-h4 text-weight-bold text-primary"
                    )
                    ui.label("Всего должностей").classes("text-caption text-grey-6")

                with ui.card().classes("flex-1 text-center p-4"):
                    ui.label(str(profiles_count)).classes(
                        "text-h4 text-weight-bold text-green"
                    )
                    ui.label(f"С профилями ({coverage_percent:.1f}%)").classes(
                        "text-caption text-grey-6"
                    )

                with ui.card().classes("flex-1 text-center p-4"):
                    ui.label(str(in_progress)).classes(
                        "text-h4 text-weight-bold text-orange"
                    )
                    ui.label("В процессе").classes("text-caption text-grey-6")

                with ui.card().classes("flex-1 text-center p-4"):
                    ui.label(str(no_profiles)).classes(
                        "text-h4 text-weight-bold text-red"
                    )
                    ui.label(f"Без профилей ({100-coverage_percent:.1f}%)").classes(
                        "text-caption text-grey-6"
                    )
    ...
    def _create_quick_actions(self):
        ...
        with ui.card().classes("w-full mb-6"):
            ...
            with ui.row().classes("w-full q-gutter-md"):
                ui.button(
                    "🔍 Найти должность", on_click=lambda: ui.navigate.to("/search")
                ...
                ui.button(
                    "⚡ Быстрая генерация", on_click=self._quick_generate
                ...
                for task in self.active_tasks[:3]:
                    with ui.row().classes("w-full items-center q-py-sm"):
                        if task.get("status") == "processing":
                            ...
                            ui.label(
                                f"⚙️ Генерируется: {task.get('current_step', 'Неизвестно')} ({progress}%)"
                            )
                        elif task.get("status") == "completed":
                            ui.icon("check_circle", color="green").classes("q-mr-sm")
                            ui.label(
                                f"✅ Создан: {task.get('position', 'Профиль')} ({self._format_time(task.get('completed_at'))})"
                            )

            # Если нет активности
            if not self.active_tasks:
                ui.label("Нет недавней активности").classes("text-grey-6 q-py-md")
                ui.label("Начните с создания первого профиля!").classes(
