...
class StatsComponent:
    ...
    def __init__(self, api_client, style: Literal["dashboard", "compact", "minimal"] = "dashboard"):
        ...
        with ui.card().classes("w-full mb-4 shadow-sm"):
            with ui.card_section().classes("p-4"):
                ...
                with ui.row().classes("items-center justify-between mb-4"):
                    ui.label("📊 Статистика системы").classes("text-h6 text-primary font-medium")
                    self.status_chip = ui.chip("Загрузка", color="grey").props("size=sm")
                
                # Основные метрики в строку - ЕДИНООБРАЗНО
                with ui.row().classes("w-full justify-between gap-4"):
                    # Должности
                    with ui.column().classes("items-center"):
                        self.positions_label = ui.label("1,553").classes("text-h4 font-bold text-primary")
                        ui.label("должностей").classes("text-body2 text-grey-6")
                    
                    # Профили
                    with ui.column().classes("items-center"):
                        self.profiles_label = ui.label("2").classes("text-h4 font-bold text-primary")
                        ui.label("создано").classes("text-body2 text-grey-6")
                    
                    # Покрытие - без прогресс бара
                    with ui.column().classes("items-center"):
                        self.progress_text = ui.label("0.1%").classes("text-h4 font-bold text-primary")
        ...
        with ui.card().classes("w-full mb-4 shadow-sm"):
            with ui.card_section().classes("p-3"):
                with ui.row().classes("items-center justify-between"):
                    ...
                    self.status_chip = ui.chip("Готов", color="positive").props("size=xs")
                
                # Единообразные метрики в строку
                with ui.row().classes("w-full justify-between mt-3"):
                    with ui.column().classes("items-center"):
                        self.positions_label = ui.label("1,553").classes("text-h6 font-bold text-primary")
                        ui.label("должностей").classes("text-caption text-grey-6")
                    
                    with ui.column().classes("items-center"):
                        self.profiles_label = ui.label("2").classes("text-h6 font-bold text-primary") 
                        ui.label("создано").classes("text-caption text-grey-6")
                    
                    with ui.column().classes("items-center"):
                        self.progress_text = ui.label("0.1%").classes("text-h6 font-bold text-primary")
    ...
    async def _render_minimal(self):
        ...
        with ui.row().classes("items-center gap-3"):
            ...
            self.positions_label = ui.label("1,553").classes("text-body2 text-white font-medium")
            ui.label("должностей").classes("text-body2 text-white opacity-60")
            ui.label("•").classes("text-white opacity-40")
            self.profiles_label = ui.label("2").classes("text-body2 text-white font-medium")
    ...
    async def _load_and_update(self):
        ...
        try:
            ...
            if data:
                ...
                self.last_data = data
                logger.debug(f"Stats updated: {data['profiles_count']} of {data['positions_count']}")
