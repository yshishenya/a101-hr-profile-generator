...
class APIClient:
  ...
  async def generate_profile(self, department: str, position: str, employee_name: str = None) -> Dict[str, Any]:
  ...
  async def get_departments_list(self, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    return await self._make_request("GET", "/api/catalog/departments", params=params)
  
  async def get_positions_for_department(self, department: str, force_refresh: bool = False) -> Dict[str, Any]:
    """
    @doc
    ...
    """
    params = {"force_refresh": force_refresh} if force_refresh else None
    return await self._make_request("GET", f"/api/catalog/positions/{department}", params=params)
...
def handle_api_error(error: APIError, context: str = "API request") -> None:
  ...
  if error.status_code == 401:
    ...
    ui.notify(f'Ошибка соединения: {error.message}', type='negative', icon='wifi_off')
