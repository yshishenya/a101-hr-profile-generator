...
async def test_full_trace_pipeline():
    ...
    try:
        ...
        def enhanced_generate(*args, **kwargs):
            ...
            logger.info(
                f"   - Variables keys: {list(args[1].keys()) if len(args) > 1 else 'N/A'}"
            )
        ...
        if result["success"]:
            ...
            if llm_meta:
                ...
                logger.info(
                    f"   - Generation time: {llm_meta.get('generation_time', 0):.2f}s"
                )
